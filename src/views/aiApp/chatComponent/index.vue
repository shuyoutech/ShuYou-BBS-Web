...
import MarkdownEventStream from './showMarkText.vue'
...
function getNewsList() {
  ...
  const params = {
    ...
    query: {
      appId: appId.value,
      conversationId: props.conversationId === '0' ? '' : props.conversationId,
    },
  }
  apiApp.newsList(params).then((res) => {
    res.data.rows.forEach((ele) => {
      const obj = {
        sender: ele.messageRole === 'user' ? 'user' : 'ai',
        content: ele.messageContent,
        editContent: ele.messageRole === 'user' ? ele.messageContent : null,
        isEdit: ele.messageRole === 'user' ? false : null,
        id: ele.id,
        createTime: ele.createTime,
      }
      messages.push(reactive(obj))
    })
  })
}
// 监听 conversationId 的变化 重新获取消息列表
...
function handleAnswer(msg, repeat = false) {
  ...
  fetchEventSource(`${baseURL}/app/chat/completions?t=${Date.now()}`, {
    method: 'POST',
    headers: {
      ...
      <div ref="chatContainer" class="question-list">
        <div v-for="(message, index) in messages" :key="index" class="message" :class="[message.sender === 'ai' ? 'ai' : 'user']">
          ...
          <div :class="{ 'ai-message': message.sender === 'ai', 'full-width': message.isEdit }">
            ...
            <div v-else>
              <MarkdownEventStream ref="answerRef" :initial-content="message.content" :interface-finish="interfaceFinish" @update-height="scrollToBottom" @notice-stop="handleShowFinish" />
