...
const title = computed(() => props.id === '' ? '新增知识库' : '编辑知识库')
...
<template>
  <div>
    ...
    <ElDrawer v-else-if="props.mode === 'drawer'" v-model="visible" :title="title" size="500px" :close-on-click-modal="false" destroy-on-close>
      <DetailForm ref="formRef" v-bind="$props" />
      <div class="buttons">
        <ElButton size="large" @click="onCancel">
          取 消
