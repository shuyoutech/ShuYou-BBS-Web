...
function onSelectFile(e: Event) {
  ...
  filesToAdd.forEach((file) => {
    ...
    return axios({
      ...
      emits('onSuccess', response.data.data, file)
    }).finally(() => {
...
<template>
  <div class="space-y-2">
    <div class="flex flex-wrap gap-2">
      <div
        v-for="(img, index) in images" :key="img"
        class="group/image-upload relative flex items-center justify-center overflow-hidden border rounded-lg" :style="{
      ...
      >
        ...
        <div v-if="!props.disabled"
             class="absolute inset-0 grid grid-cols-2 place-items-center rounded-lg bg-black/50 p-2 text-white opacity-0 transition-opacity group-hover/image-upload:opacity-100">
          <div class="flex-center cursor-pointer opacity-60 transition-all hover:(scale-110 opacity-100)"
               @click.prevent="onPreview(index)">
      ...
      <button
        v-if="images.length < props.max || props.max === 0" type="button"
        class="relative flex-center overflow-hidden bg-transparent" :class="{
      ...
      >
        ...
        <input ref="fileInputRef" type="file" accept="image/*" :multiple="props.multiple" class="hidden"
               @change="onSelectFile">
      </button>
    </div>
