...
const searchDefault = {
  dictLabel: '',
  dictTypeId: route.params.id as string ?? '',
}
...
function getDataList() {
  ...
  const params = {
    ...
    order: 'dictSort',
    query: {
      dictLabel: search.value.dictLabel,
      dictTypeId: route.query.id as string ?? '',
    },
  }
  apiDict.itemList(params).then((res: any) => {
    loading.value = false
...
function onDel(row: any) {
  ElMessageBox.confirm(`确认删除「${row.dictLabel}」吗？`, '确认信息').then(() => {
    apiDict.itemDelete([row.id]).then(() => {
      getDataList()
      toast.success('删除成功')
    })
  }).catch(() => {})
  ...
  <div :class="{ 'absolute-container': tableAutoHeight }">
    <FaPageMain :class="{ 'flex-1 overflow-auto': tableAutoHeight }" :main-class="{ 'flex-1 flex flex-col overflow-auto': tableAutoHeight }">
      <FaSearchBar :show-toggle="false">
        <ElForm :model="search" size="default" label-width="100px" inline-message inline class="search-form">
          <ElFormItem label="字典标签">
            <ElInput v-model="search.dictLabel" placeholder="请输入字典标签" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem>
      ...
      <ElTable v-loading="loading" class="my-4" :data="dataList" stripe highlight-current-row border height="100%" @sort-change="sortChange">
        ...
        <ElTableColumn prop="dictSort" sortable label="字典排序" />
    ...
    <FormMode v-if="formMode === 'dialog' || formMode === 'drawer'" :id="formModeProps.id" v-model="formModeProps.visible" :mode="formMode" type="dictDataList" @success="getDataList" />
  </div>
</template>
