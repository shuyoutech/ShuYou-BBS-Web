...
function getDataList() {
  ...
  apiDict.list(params).then((res: any) => {
    loading.value = false
...
function onDel(row: any) {
  ElMessageBox.confirm(`确认删除「${row.dictName}」吗？`, '确认信息').then(() => {
    apiDict.delete([row.id]).then(() => {
      getDataList()
      toast.success('删除成功')
    })
  }).catch(() => {})
  ...
  <div :class="{ 'absolute-container': tableAutoHeight }">
    <FaPageMain :class="{ 'flex-1 overflow-auto': tableAutoHeight }" :main-class="{ 'flex-1 flex flex-col overflow-auto': tableAutoHeight }">
      <FaSearchBar :show-toggle="false">
        <ElForm :model="search" size="default" label-width="100px" inline-message inline class="search-form">
          <ElFormItem label="字典名称">
            <ElInput v-model="search.dictName" placeholder="请输入字典名称" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem label="字典类型">
            <ElInput v-model="search.dictCode" placeholder="请输入字典类型" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem>
      ...
      <ElTable v-loading="loading" class="my-4" :data="dataList" stripe highlight-current-row border height="100%" @sort-change="sortChange">
        ...
        <ElTableColumn prop="dictCode" label="字典类型">
          <template #default="scope">
            <span class="click-btn" @click="handleDetail(scope.row)">{{ scope.row.dictCode }}</span>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="dictDesc" label="字典描述" />
