...
function getDataList() {
  ...
  const params = {
    ...
    order: '',
    query: {
      noticeType: search.value.noticeType,
      noticeTitle: search.value.noticeTitle,
    },
  }
  apiNotice.list(params).then((res: any) => {
    loading.value = false
...
function onDel(row: any) {
  ElMessageBox.confirm(`确认删除「${row.noticeTitle}」吗？`, '确认信息').then(() => {
    apiNotice.delete([row.id]).then(() => {
      getDataList()
      toast.success('删除成功')
    })
  }).catch(() => {})
...
function onChangeStatus(row: any) {
  return new Promise<boolean>((resolve) => {
    ElMessageBox.confirm(`确认${row.status === '1' ? '禁用' : '启用'}「${row.noticeTitle}」吗？`, '确认信息').then(() => {
      row.statusLoading = true
      apiNotice.changeStatus({
        id: row.id,
        status: row.status === '1' ? '0' : '1',
      }).then(() => {
        row.statusLoading = false
        toast.success(`${row.status === '1' ? '禁用' : '启用'}成功`)
        return resolve(true)
      }).catch(() => {
        row.statusLoading = false
  ...
  <div :class="{ 'absolute-container': tableAutoHeight }">
    <FaPageMain :class="{ 'flex-1 overflow-auto': tableAutoHeight }" :main-class="{ 'flex-1 flex flex-col overflow-auto': tableAutoHeight }">
      <FaSearchBar :show-toggle="false">
        <ElForm :model="search" size="default" label-width="100px" inline-message inline class="search-form">
          <ElFormItem label="公告标题">
            <ElInput v-model="search.noticeTitle" placeholder="请输入公告标题" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem label="公告类型">
            <ElSelect v-model="search.noticeType" placeholder="请选择公告类型">
      ...
      <ElTable v-loading="loading" class="my-4" :data="dataList" stripe highlight-current-row border height="100%" @sort-change="sortChange">
        ...
        <ElTableColumn prop="noticeType" label="公告类型">
          <template #default="scope">
            {{ scope.row.noticeType === '1' ? '通知' : '公告' }}
          </template>
        </ElTableColumn>
        <ElTableColumn prop="noticeDesc" label="公告描述" />
        <ElTableColumn label="状态" width="100" align="center">
          <template #default="scope">
            <ElSwitch v-model="scope.row.status" active-value="1" inactive-value="0" inline-prompt active-text="启用" inactive-text="禁用" :before-change="() => onChangeStatus(scope.row)" />
          </template>
        </ElTableColumn>
