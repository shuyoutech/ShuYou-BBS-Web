...
function getDataList() {
  ...
  const params = {
    ...
    order: '',
    query: {
      knowledgeName: search.value.name,
    },
  }
...
function onDel(row: any) {
  ElMessageBox.confirm(`确认删除「${row.knowledgeName}」吗？`, '确认信息').then(() => {
    apiDatabase.delete([row.id]).then(() => {
      getDataList()
      toast.success('删除成功')
    })
  }).catch(() => {})
...
<template>
  <div class="p-20px">
    <ElInput v-model="search.name" class="mr-20px w-200px" placeholder="搜索知识库" clearable @clear="handleClear" />
