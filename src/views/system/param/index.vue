...
function getDataList() {
  ...
  const params = {
    ...
    order: '',
    query: {
      configName: search.value.configName,
      configKey: search.value.configKey,
    },
  }
  apiParam.list(params).then((res: any) => {
    loading.value = false
...
function onDel(row: any) {
  ElMessageBox.confirm(`确认删除「${row.configName}」吗？`, '确认信息').then(() => {
    apiParam.delete([row.id]).then(() => {
      getDataList()
      toast.success('删除成功')
    })
  }).catch(() => {})
  ...
  <div :class="{ 'absolute-container': tableAutoHeight }">
    <FaPageMain :class="{ 'flex-1 overflow-auto': tableAutoHeight }" :main-class="{ 'flex-1 flex flex-col overflow-auto': tableAutoHeight }">
      <FaSearchBar :show-toggle="false">
        <ElForm :model="search" size="default" label-width="100px" inline-message inline class="search-form">
          <ElFormItem label="参数名称">
            <ElInput v-model="search.configName" placeholder="请输入参数名称" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem label="参数键名">
            <ElInput v-model="search.configKey" placeholder="请输入参数键名" clearable @keydown.enter="currentChange()" @clear="currentChange()" />
          </ElFormItem>
          <ElFormItem>
      ...
      <ElTable v-loading="loading" class="my-4" :data="dataList" stripe highlight-current-row border height="100%" @sort-change="sortChange">
        <ElTableColumn prop="configName" label="参数名称" />
        <ElTableColumn prop="configKey" label="参数键名" />
        <ElTableColumn prop="configValue" label="参数键值" />
        <ElTableColumn prop="configDesc" label="描述" />
